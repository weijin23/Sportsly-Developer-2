...
- (BOOL)requestPolylineFromPoint:(CLLocation *)fromPoint toPoint:(CLLocation *)toPoint andMode:(NSString*)mode
{
    if (!fromPoint || !toPoint) return FALSE;
    
    NSString *urlString = [NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/directions/json?origin=%0.06f,%0.06f&destination=%0.06f,%0.06f&sensor=true&mode=%@", fromPoint.coordinate.latitude, fromPoint.coordinate.longitude, toPoint.coordinate.latitude, toPoint.coordinate.longitude,mode];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    _responseData = nil;
      NSError* error;
     NSMutableDictionary *response= [NSJSONSerialization JSONObjectWithData: [stringResponse dataUsingEncoding:NSUTF8StringEncoding] options:kNilOptions error:&error];
    ...
    for (NSDictionary *route in routes)
    {
        ...
        for (NSDictionary *leg in legs)
        {
            ...
            for (NSDictionary *step in steps)
            {
                ...
                gstep.lastAddressString=@"End of route";
                gstep.htmlInstructions =[self stringByStrippingHTML:[step valueForKey:@"html_instructions"]];
