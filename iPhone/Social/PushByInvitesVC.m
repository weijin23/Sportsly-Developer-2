...
@implementation PushByInvitesVC
...
- (void)viewDidLoad
{
    ...
    if(delegate)
    {
    if([delegate respondsToSelector:@selector(didChangeNumberOfInvites:)])
    {
        [delegate didChangeNumberOfInvites:[NSString stringWithFormat:@"%i",self.fetchedResultsController.fetchedObjects.count]];
    }
    }
...
{
   UITableViewCell *cell=cell1;
    
    
    
    Invite *newEvent= [fetchedResultsController objectAtIndexPath:indexPath];
...
//    cell.detailslabTime.text = [appDelegate.dateFormatM stringFromDate:newEvent.startTime ];
    ...
    cell1.textLabel.text=newEvent.message;
   /* cell.detailslabName.text =newEvent.eventName;
    cell.detailslabField.text =newEvent.fieldName;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   if(newEvent.type.intValue==0 || newEvent.type.intValue==4 )
   {
    if(delegate)
    {
    if([delegate respondsToSelector:@selector(didSelectInvite::)])
    {
        [delegate didSelectInvite:newEvent:self.popOver];
    }
    }
   }
    else //if(newEvent.type.intValue==1)
    {
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    NSPredicate *pre=[NSPredicate predicateWithFormat:@"type!=%i && type!=%i && type!=%i && type!=%i && type!=%i",4,3,0,1,2];
    
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"teamId" ascending:YES /*selector:@selector(localizedCaseInsensitiveCompare:)*/];
