...
- (IBAction)signInBtn:(id)sender
{
    ...
    NSLog(@"RequestParamJSON=%@",jsonCommand);
        ...
        [appDelegate sendRequestFor:VERIFY from:self parameter:[NSDictionary dictionaryWithObjectsAndKeys:jsonCommand,@"requestParam", nil]];
}
...
-(void)notifyRequesterWithData:(id) aData :(id)aData1
{
    ...
    if([aData1 isKindOfClass:[ConnectionManager class]])
    {
        ConnectionManager *aR=(ConnectionManager*)aData1;
        if([aR.requestSingleId isEqualToString:VERIFY])
        {
            NSString *response = (NSString *)aData;
            response = [response stringByReplacingOccurrencesOfString:@" " withString:@""];
            if([response isEqualToString:@"success"])
            {
                UIAlertView *al=[[UIAlertView alloc] initWithTitle:@"" message:@"Verification email has been sent." delegate:self cancelButtonTitle:@"Okay" otherButtonTitles: nil];
                [al show];
            }
        }
        return;
